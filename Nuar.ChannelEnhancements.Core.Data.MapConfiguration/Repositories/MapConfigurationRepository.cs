...
public class MapConfigurationRepository : QueryBase, IMapConfigurationRepository
{
    ...
    /// <summary>
    /// Initializes an instance of <see cref="MapConfigurationRepository"/>.
    /// </summary>
    /// <param name="dbSettings">The database connection configuration.</param>
    /// <param name="mapViewRepository">The <see cref="IMapViewRepository"/> repository</param>
    /// <param name="layerGroupRepository">The <see cref="ILayerGroupRepository"/> repository</param>
    /// <param name="layerRepository">The <see cref="ILayerRepository"/> repository</param>
    /// <param name="attributeGroupRepository">The <see cref="IAttributeGroupRepository"/> repository</param>
    /// <param name="styleRepository">The <see cref="IStyleRepository"/> repository</param>
    /// <exception cref="ArgumentNullException"></exception>
    public MapConfigurationRepository(DatabaseConfigurationSettings dbSettings, IMapViewRepository mapViewRepository, ILayerGroupRepository layerGroupRepository, ILayerRepository layerRepository, IAttributeGroupRepository attributeGroupRepository, IStyleRepository styleRepository) : base(
        dbSettings.ConnectionString ??
        throw new ArgumentNullException(nameof(dbSettings.ConnectionString)))
    {
        ...
        // Register the custom mappings for the MapConfiguration model
        SqlMapper.SetTypeMap(typeof(Models.MapConfiguration), new ColumnAttributeTypeMapper<Models.MapConfiguration>());

        // Initialize the repositories with the provided dependencies
    ...
    public Task<IEnumerable<Models.MapConfiguration>?> GetAll()
    {
        try
        {
            ...
            var mapConfigs = Task.FromResult<IEnumerable<Models.MapConfiguration>?>(Connection.Query<Models.MapConfiguration>(
                sql: $"{_dbSettings.MapConfigurationSchemaName}.{_dbSettings.MapConfigurationFunctionMap["GetMapConfigs"]}",
                param: null,
        ...
        {
            Log.Error($"{nameof(MapConfigurationRepository.GetAll)} - Unable to retrieve list of map configurations. : {ex.Message}");
            return Task.FromResult<IEnumerable<Models.MapConfiguration>?>(null);
        }
    }
    ...
    public Task<Models.MapConfiguration?> GetMapConfigById(Guid Id)
    {
        try
        {
            ...
            var mapConfig = Task.FromResult(Connection.Query<Models.MapConfiguration>(
                sql: $"{_dbSettings.MapConfigurationSchemaName}.{_dbSettings.MapConfigurationFunctionMap["GetMapConfigById"]}",
                param: p,
                commandType: CommandType.StoredProcedure,
        ...
        {
            Log.Error($"{nameof(MapConfigurationRepository.GetMapConfigById)} - Unable to retrieve map configuration: {Id}.  {ex.Message}");
            return Task.FromResult<Models.MapConfiguration?>(null);
        }
    }
