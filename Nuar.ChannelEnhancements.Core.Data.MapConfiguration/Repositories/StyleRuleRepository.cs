...
public class StyleRuleRepository : QueryBase, IStyleRuleRepository
{
    ...
    /// </summary>
    /// <param name="dbSettings">The platform database configuration.</param>
    public StyleRuleRepository(DatabaseConfigurationSettings dbSettings) : base(dbSettings.ConnectionString ?? throw new ArgumentNullException(dbSettings.ConnectionString))
    {
        ...
        // Register the custom mappings for the StyleRule model
        SqlMapper.SetTypeMap(typeof(StyleRule), new ColumnAttributeTypeMapper<StyleRule>());
    }
    ...
    public async Task<IEnumerable<StyleRule>?> GetAll()
    {
        try
        {
            ...
            var styleRules = await Connection.QueryAsync<StyleRule>(
                sql: $"{_dbSettings.MapConfigurationSchemaName}.{_dbSettings.MapConfigurationFunctionMap["GetAllStyleRules"]}",
                param: null,
    ...
    public async Task<IEnumerable<StyleRule>?> GetAllByLayerId(Guid layerId)
    {
        try
        {
            ...
            var styleRules = await Connection.QueryAsync<StyleRule>(
                sql: $"{_dbSettings.MapConfigurationSchemaName}.{_dbSettings.MapConfigurationFunctionMap["GetStyleRulesByLayerId"]}",
                param: p,
                commandType: CommandType.StoredProcedure,
                commandTimeout: 900) as List<StyleRule>;
    ...
    public async Task<StyleRule?> GetRuleById(Guid styleRuleId)
    {
        try
        {
            ...
            var styleRule = await Connection.QueryAsync<StyleRule>(
                sql: $"{_dbSettings.MapConfigurationSchemaName}.{_dbSettings.MapConfigurationFunctionMap["GetStyleRuleById"]}",
                param: p,
                commandType: CommandType.StoredProcedure,
                commandTimeout: 900);
    ...
    private async Task<IEnumerable<StyleRuleCondition>?> GetStyleRuleConditions(Guid styleRuleId)
    {
        try
        {
            ...
            var conditions = await Connection.QueryAsync<StyleRuleCondition>(
                sql: $"{_dbSettings.MapConfigurationSchemaName}.{_dbSettings.MapConfigurationFunctionMap["GetStyleRuleConditionsByRuleId"]}",
                param: p,
                commandType: CommandType.StoredProcedure,
